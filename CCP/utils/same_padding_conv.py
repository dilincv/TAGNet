# modify from https://github.com/Gasoonjia/Tensorflow-type-padding-with-pytorch-conv2d./blob/master/Conv2d_tensorflow.py
# some bugs in ori impl
# ---------------
...
class Conv2d(_ConvNd):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=(1, 1),
                 padding='SAME', dilation=(1, 1), groups=1, bias=True, weight=None):
...
def conv2d_same_padding(input, weight, bias=None, stride=1, padding='SAME', dilation=1, groups=1):
    ...
    if padding == 'SAME':
        ...
        input = pad(input,
                    (
                        padding_cols // 2, padding_cols // 2 + int(cols_odd),
...
class SeparableConv2dSAME(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=(1, 1), padding='SAME', dilation=(1, 1),
                 bias=False):
        super(SeparableConv2dSAME, self).__init__()

        self.conv1 = Conv2d(in_channels, in_channels, kernel_size, stride, padding, dilation, groups=in_channels,
                            bias=bias)
        self.pointwise = Conv2d(in_channels, out_channels, 1, 1, 'SAME', 1, 1, bias=bias)
